...
describe('GameService', () => {
  ...
  it('expects service to fetch a week\'s games', () => {
    ...
    service.getWeekGames(2018, 13).subscribe((games: any) => {
      expect(games['data'].length).toBe(5);
      expect(games['data'].map(game => { return game.week })).toEqual([13, 13, 13, 13, 13]);
    });
  ...
  it('expects service to fetch a team\'s games', () => {
    ...
    service.getTeamGames(54).subscribe((games: any) => {
      ...
      expect(teams.length).toEqual(15);
...
// Hang on to for now. Might want to reference later.
  ...
  //
  //       // Call the service
  //       service.getWeekGames(2018, 13).subscribe(res => {
  //         console.log(res)
